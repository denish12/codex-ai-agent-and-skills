...
2. Задаёт вопросы по темам:
   ...
   - Какой режим тестирования? (a) Antigravity Browser — визуальная проверка через встроенный браузер (`$qa_browser_testing`), (b) Playwright CI/CD — автоматизированные E2E spec-файлы (`$qa_e2e_playwright`)
   **Минимум:** 5 вопросов.
...
> **Триггер:** пользователь добавляет слово **«Wix»** или **«Shopify»** при переходе к TEST gate.
> Пример: _"Approved. TEST gate. Wix."_ или _"Следующий: TEST. Shopify."_

Когда приложение работает внутри **закрытой экосистемы** (Wix Dashboard, Shopify Admin и т.п.),
прямой доступ к `localhost` из sandbox-браузера невозможен. Используется **Pre-Auth Handoff:**

### Протокол (обязательный при наличии триггера)

**Шаг 1 — Tester запрашивает pre-auth:**
```
...
**Шаг 2 — Пользователь авторизуется и открывает страницу.**

**Шаг 3 — Tester запускает `browser_subagent` со строгими правилами:**
- ❌ НЕ открывать новые URL
- ❌ НЕ пытаться навигировать на localhost
- ✅ Работать ТОЛЬКО с уже открытой страницей
- ✅ Делать скриншоты на каждом шаге
- ✅ Кликать, переключать, проверять UI
- ✅ Записать видео (`.webp`) для evidence

**Шаг 4 — Tester анализирует скриншоты** и добавляет результаты в секцию «DEMO Results» отчёта.

### Что проверять через Pre-Auth Handoff
- Визуальное соответствие: экраны, компоненты, лейауты
- Навигация: переходы между страницами/вкладками
- Интерактивность: клики, ховеры, переключатели
- Live Preview: рендеринг, обновление, viewport toggle
- Регрессия: ранее работавшие функции не сломаны

### Артефакты Pre-Auth Handoff
- Скриншоты (`.png`) — embed в walkthrough.md
- Видео-запись (`.webp`) — embed в walkthrough.md
- DEMO Results таблица — в TEST report

### Ограничения
- `localhost` / self-signed SSL — **не работает** (sandbox-ограничение)
- Внешние URL с валидным SSL — **работает** (manage.wix.com, admin.shopify.com, etc.)
- Браузер **общий** — auth state (cookies, session) сохраняется между пользователем и субагентом

### Если Pre-Auth не выполнен
Если пользователь не предоставил pre-auth сессию, Tester:
1. Пометить визуальную верификацию как `🟡 MANUAL — delegated to user`
2. Предоставить чеклист для ручной проверки
3. НЕ блокировать отчёт — unit-тесты + code analysis достаточны для промежуточного отчёта
...
- $qa_test_plan
- $qa_manual_run
- $qa_browser_testing — визуальное E2E через встроенный Antigravity Browser
- $qa_e2e_playwright — автоматизированный E2E для CI/CD pipeline
