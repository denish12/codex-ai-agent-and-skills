...
description: Observability plan — what logs/metrics/traces to collect, request correlation, log levels, alerts. Without leaking PII/secrets.
---

# Skill: Observability Plan

Architectural observability plan: what to log, what to measure, when to alert.
...
## See also
- `$observability-logging` — implementation (pino setup, middleware, redaction code)
- `$architecture-doc` — Architecture Document (observability section)
- `$api-contracts` — error codes (map to UI states)
- `$threat-model-baseline` — audit logging requirements
