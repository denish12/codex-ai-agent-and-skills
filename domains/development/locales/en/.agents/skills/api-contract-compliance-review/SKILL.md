---
name: api-contract-compliance-review
description: API implementation contract compliance review — schemas, error codes, validation, authorization, idempotency, pagination.
---

# Skill: API Contract Compliance Review

Review: API implementation conforms to the declared contracts.
...
```
1. Load API Contracts (from $api-contracts)
   └── List of endpoints, schemas, error codes

2. For each endpoint:
   a. Find the implementation file (route → controller → service)
...
## See also
- `$api-contracts` — API contract definitions (source of truth)
- `$code-review-checklist` — general review (API section)
- `$security-review` — auth/validation depth (SEC-06..SEC-17)
- `$review-reference-snippets` — DO/DON'T snippets (section M)
