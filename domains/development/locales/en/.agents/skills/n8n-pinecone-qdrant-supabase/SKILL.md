...
**Sections:**
1. [n8n: workflows patterns](#1-n8n-workflows)
...
### ✅ DO: reliable workflow structure
```
...
- **Action**: HTTP Request / native node / external API call
- **Error Handler**: always attach to the workflow — otherwise a failed run will silently disappear

### ✅ DO: Code node for transformations (JS)
```js
// n8n Code node — processes all items
const results = [];
...
for (const item of $input.all()) {
  ...
  results.push({
    json: {
      id:       sourceId,
      content:  text.trim().slice(0, 8000), // ✅ truncate to embedding limit
      metadata: {
        ...metadata,
...
function chunkText(text, chunkSize = 1000, overlap = 200) {
  ...
  while (start < text.length) {
    ...
    start += chunkSize - overlap; // ✅ overlap preserves context at the boundaries
  }
...
### ✅ DO: error handling in workflow
```
1. Settings → Error Workflow → specify a separate error-workflow
2. In error-workflow: get $execution.error → log to Supabase/Slack
3. For HTTP Request nodes: enable "Continue on Fail" + check status in the next node
```

```js
// Code node: checking HTTP Request response
...
```
Settings → Variables → add PINECONE_API_KEY, SUPABASE_URL, etc.
In nodes: {{ $vars.PINECONE_API_KEY }}
❌ Never hardcode keys directly into nodes
```
...
### ✅ DO: namespace strategy
```
namespace = tenant or data category

Examples:
  "user_123"          → user's personal data
  "docs_product"      → product documentation
  "kb_support"        → support knowledge base
  ""                  → default namespace (do not mix with named ones)
```

### ✅ DO: embeddings upsert (HTTP Request node in n8n)
```js
// Code node: preparing a batch for Pinecone upsert
const items = $input.all();

const vectors = items.map(item => ({
  id:       item.json.id,          // unique string ID
  values:   item.json.embedding,   // float[] from the embedding model
  metadata: {
    // ✅ Only primitives: string, number, boolean, string[]
    text:       item.json.text.slice(0, 1000), // Pinecone metadata limit ~40KB
...
// Return in batches of 100 (Pinecone upsert limit)
const batchSize = 100;
const batches = [];
...
```js
// Code node: processing search results
const matches = $input.first().json.matches ?? [];

// ✅ Filter by score — cut off weak matches
...
### ✅ DO: creating a collection with correct parameters
```json
...
```
distance:
  Cosine   → normalized embeddings (OpenAI, text-embedding-*)  ✅ most often
  Dot      → not normalized, maximize scalar product
  Euclid   → when absolute distance is important
```

### ✅ DO: upserting points with payload
```js
...
const points = $input.all().map(item => ({
  ...
  payload: {
    ...
    createdAt:  Math.floor(Date.now() / 1000), // ✅ unix timestamp for range-filters
  },
}));
...
// Use when you need to iterate over ALL points without vector search
{
  "limit":        100,
  "with_payload": true,
  "with_vector":  false,
...
### ✅ DO: named vectors for multi-modal search
```json
...
create table documents (
  ...
  embedding   vector(1536),          -- size matching the model
...
-- Service key (service_role) bypasses RLS — use only on backend/n8n
-- Anon/authenticated key — obeys RLS — use on the client
```

### ✅ DO: calling match_documents from n8n (HTTP Request)
```json
...
```
Supabase → Database Webhooks → on INSERT/UPDATE in table → POST to n8n Webhook URL
...
```js
// n8n HTTP Request: uploading a file to Supabase Storage
...
// After upload — save metadata to DB
...
[Fetch Document] — HTTP/Supabase Storage
        ↓
[Extract Text] — Code node (PDF→text, HTML→text)
        ↓
[Chunk Text] — Code node (1000 chars, overlap 200)
        ↓
...
[Filter & Rank] — Code node: score >= 0.75, top 5
        ↓
[Build Prompt] — Code node: assembling context + question
        ↓
[LLM Generation] — OpenAI /v1/chat/completions
        ↓
[Format Response] — Code node: answer + sources
        ↓
[Save to History] — Supabase (question, answer, sources)
        ↓
...
const messages = [
  {
    role: "system",
    content: `You are an assistant. Answer ONLY based on the provided context.
If there is no answer in the context — say "Information not found".
Context:
${context}`,
  },
  {
...
```sql
-- Supabase: combining semantic and full-text search (RRF fusion)
...
// ✅ Prefer: count=exact — will return Content-Range header with total count
```
...
// ✅ merge-duplicates — will update existing, create new if not present
// ignore-duplicates — will skip if already present
```

### ✅ DO: listen to Realtime via n8n Webhook
```
Supabase Realtime → Database Webhook → n8n:

Scenarios:
  INSERT into tasks       → trigger processing workflow
  UPDATE status='done'    → send notification to user  
  DELETE from documents   → delete embeddings from Pinecone/Qdrant
```

### ✅ DO: error management + retry
```js
// Code node after HTTP Request to Supabase
const { statusCode, body } = $input.first().json;

if (statusCode === 409) {
  // Conflict — record already exists, this is OK
  return [{ json: { status: "already_exists", ...body } }];
}

if (statusCode >= 400) {
  // ✅ Throw with details — will end up in error workflow
...
anon         → obeys RLS, limited         → client code ✅

❌ service_role in browser/mobile app — leak = full access to DB
❌ anon in n8n for operations requiring RLS bypass — will result in 403
```

### ❌ DON'T: store the entire text in Pinecone metadata
```
❌ Pinecone metadata limit ~40KB per vector
❌ Slow filtering by long strings

✅ Store in metadata only: id, short preview (200 chars), tags, dates
✅ Full text — in Supabase/PostgreSQL, retrieve by id after search
```
...
// ✅ Batch upsert
// Pinecone: up to 100 vectors at a time
// Qdrant:   up to 1000 points at a time (wait=false for async)
// In n8n: Split In Batches node → HTTP Request
```
...
❌ Create a collection for 1536, insert ada-002, then switch to 3-large (3072) — error
✅ Lock the model and size in config, do not change without recreating the collection
```

### ❌ DON'T: run a workflow without idempotency
```js
// ❌ Bad: repeated run duplicates data
// ✅ Always use upsert, not insert
// ✅ Add an idempotent key (docId + chunkIndex → deterministic ID)

// ✅ Deterministic ID for chunk (same on repeated run)
function makeChunkId(docId, chunkIndex) {
  return `${docId}_chunk_${chunkIndex}`;
}
// Pinecone: id = makeChunkId(...)
// Qdrant:   id = uuid5(namespace, makeChunkId(...)) — if UUID is needed
```
...
Supabase REST:     depends on plan, default 500 req/s

✅ In n8n: Split In Batches + Wait node between batches
...
## See also
- `dev-reference-snippets` — Express API, validation, error handling
- `es2025-beast-practices` — async/await patterns, Promise.all for parallel requests
