...
| # | Check | Severity | Status |
|---|-------|----------|--------|
| REV-05 | Code follows `$architecture-doc` layers | 🔴 P0 | ☐ |
| REV-06 | API implementation matches `$api-contracts` exactly | 🔴 P0 | ☐ |
...
| **Ignoring the contract** | The code might be beautiful but if it returns HTTP 200 instead of 201 as per `$api-contracts`, it's a bug. |
| **Skipping security checks** | Always check for input validation and auth boundaries. |

---

## See also
- `$api-contract-compliance-review` — Deeper dive into API contracts.
- `$architecture-compliance-review` — Deeper dive into layer boundaries.
- `$security-review` — Deeper dive into AppSec.
- `$tests-quality-review` — Deeper dive into test quality.
