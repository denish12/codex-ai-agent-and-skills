---
name: ui-a11y-smoke-review
description: Quick UI/a11y smoke review by a designer — loading/empty/error states, focus, keyboard, aria/labels, compliance with UX Spec. Executed BEFORE handoff to QA.
---

# Skill: UI & A11y Smoke Review (Designer-Level)

Quick designer-level check of UI for compliance with UX Spec + a11y baseline **before handoff to QA**.

> **Difference from `$qa-ui-a11y-smoke`:** This skill is a designer's quick check (5-10 min, against UX Spec).
> `$qa-ui-a11y-smoke` is a formal QA test (25 WCAG checks, tools, report).
...
1. Input:
   ...
   ├── UX Spec (from $ux-spec) — source of truth
   └── A11y Baseline (from $a11y-baseline) — requirements
...
Note: This is a QUICK check. For formal testing → $qa-ui-a11y-smoke
```
...
## Next
- If READY → pass to $qa-ui-a11y-smoke for formal testing
...
## See also
- `$qa-ui-a11y-smoke` — formal QA a11y testing (25 WCAG checks)
- `$ux-spec` — UX Spec (source of truth)
- `$a11y-baseline` — a11y requirements
- `$design-parity-review` — full design comparison
