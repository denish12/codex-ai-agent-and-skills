...
Ревью: реализация API соответствует заявленным контрактам.
...
```
1. Загрузи API Contracts (из $api-contracts)
   └── Список endpoints, schemas, error codes

2. Для каждого endpoint:
   a. Найди файл реализации (route → controller → service)
   b. Сравни: method, path, request schema, response schema
   c. Проверь: error format, status codes, auth, validation

3. Проверь cross-cutting concerns:
   └── Error format, pagination, auth, rate limiting

4. Сформируй compliance report
```
