---
name: qa-test-plan
description: Составить тест-план и матрицу покрытия по PRD/UX — роли, критичные user flows, состояния UI, негативные кейсы, критерии приёмки.
---

# Skill: QA Test Plan

Тест-план с coverage matrix по PRD/UX flows.
...
```
1. Прочитай PRD / UX Spec / Architecture Doc
   └── Выдели user flows и acceptance criteria

2. Определи scope:
   ├── Что тестируем (in scope)
   └── Что НЕ тестируем (out of scope + почему)

3. Разбей на test suites по категориям:
...
4. Для каждого flow → test cases (section 3)

5. Приоритизация (section 4)

6. Определи entry/exit criteria (section 5)
```
...
## Test Suites
1. API Contract Tests (8 cases) → $qa-api-contract-tests
2. Manual Functional (12 cases) → $qa-manual-run
3. Security Smoke (6 cases) → $qa-security-smoke-tests
4. UI/a11y Smoke (4 cases) → $qa-ui-a11y-smoke
...
## См. также
- `$qa-manual-run` — execute manual test cases
- `$qa-api-contract-tests` — API contract test execution
- `$qa-security-smoke-tests` — security smoke execution
- `$qa-ui-a11y-smoke` — UI/a11y smoke execution
- `$qa-browser-testing` — browser-based visual testing
