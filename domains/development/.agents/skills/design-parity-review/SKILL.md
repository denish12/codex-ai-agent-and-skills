...
Сверка дизайна с UX Spec и/или реализованным UI.

**Разделы:**
1. [Workflow](#1-workflow)
2. [Review Modes](#2-modes)
3. [Comparison Table](#3-comparison)
4. [Severity Guide](#4-severity)
5. [Пример: Settings Screen](#5-пример)
...
```
1. Input:
   ├── Design Reference Map + Design Rules (из $design-intake)
   ├── UX Spec (из $ux-spec) — для Mode A
   └── Implemented UI (screenshots/browser) — для Mode B
...
## См. также
- `$design-intake` — design sources inventory (input)
- `$ux-spec` — UX Spec (comparison target)
- `$ui-inventory` — component inventory
- `$qa-browser-testing` — capture screenshots for comparison
