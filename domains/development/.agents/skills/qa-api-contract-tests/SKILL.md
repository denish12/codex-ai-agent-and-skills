...
description: Проверить API на соответствие контрактам — схемы, коды, ошибки, валидация, пагинация; smoke через curl/скрипты.
---

# Skill: QA API Contract Tests

Прогон API smoke tests: endpoints, status codes, error format, validation.
...
3. Заполни results table
4. Для каждого FAIL → bug report
```
...
## См. также
- `$api-contracts` — contract definitions (source of truth)
- `$api-contract-compliance-review` — code review of implementation
- `$qa-test-plan` — test plan (references this skill)
- `$qa-security-smoke-tests` — security-focused API tests
