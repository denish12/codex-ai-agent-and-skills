---
name: qa-ui-a11y-smoke
description: UI/a11y smoke — клавиатура/фокус, labels/aria, контраст базово, состояния форм и списков, соответствие UX Spec.
---

# Skill: QA UI & A11y Smoke

Базовая проверка UI accessibility и usability.
...
1. Автоматическая проверка
   ...
   └── Зафиксируй score и critical issues

2. Keyboard navigation (section 3)
   └── Пройди все interactive элементы Tab'ом

3. Visual check
   ├── UI states (loading, empty, error, success)
   ├── Responsive (если в scope)
   └── Match UX Spec

4. ARIA / Forms check (section 2)
   └── Labels, roles, error messages

5. Заполни output template
```
...
## См. также
- `$a11y-baseline` — a11y baseline requirements
- `$ui-a11y-smoke-review` — designer-level a11y review
- `$qa-manual-run` — general manual testing
- `$qa-browser-testing` — browser-based visual testing
- `$styling-css-stack` — CSS a11y patterns (focus-visible, prefers-reduced-motion)
