---
name: docker-kubernetes-architecture
...
Контейнеризация и оркестрация: от Dockerfile до production K8s.
...
| Vercel/Netlify/serverless only | ⬜ Не нужен |
| Микросервисы / несколько компонентов | ✅ Full skill |
| Горизонтальное масштабирование | ✅ K8s section |
...
## См. также
- `$deployment-ci-plan` — CI pipeline, environments, migrations
- `$k8s-manifests-conventions` — manifest naming, labels, repo structure
- `$observability-plan` — detailed logging/metrics plan
- `$threat-model-baseline` — security requirements
