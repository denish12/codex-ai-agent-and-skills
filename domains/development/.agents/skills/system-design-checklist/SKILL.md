...
## См. также
- `$current-state-analysis` — аудит текущего состояния (перед этим checklist)
- `$architecture-doc` — полный Architecture Document (после checklist)
- `$adr-log` — фиксация решений
- `$security-baseline-dev` — детальная security проверка
