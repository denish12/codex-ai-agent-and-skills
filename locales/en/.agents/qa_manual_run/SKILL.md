...
Manual run test cases with documenting results.
...
2. Run
   ...
   ├── For each: execution steps, check expected result
   ├── Record: PASS / FAIL / BLOCKED / SKIPPED
   └── For FAIL → create bug report (section 3)

3. Regression
   └── Rerun previously closed bugs (check that the fix did not break anything)

4. Report
   └── Fill in output template (section 5)
```
