...
8. [Organization files](#8-organize)
...
### ✅ DO: CSS custom properties how unified source truth
...
```css
/* ❌ Magic values — impossible support, no dark mode */
.card {
  padding: 17px;
  color: #333;
  background: white;
...
/* ? Tokens ? consistent, supports theming */
.card {
  padding: var(--space-4);
...
```html
<div class="coupon-card coupon-card--active is-selected">
  <h3 class="coupon-card__title">SAVE20</h3>
  <span class="coupon-card__code">20% off</span>
...
function CouponCard({ coupon }) {
  return (
    <div className={clsx(styles.card, coupon.active && styles.active)}>
      <h3 className={styles.title}>{coupon.code}</h3>
      <span className={styles.code}>{coupon.discount}%</span>
    </div>
  );
}
```
...
| Approach | When ✅ | When ❌ |
|--------|---------|---------|
| **BEM** | Vanilla CSS/HTML, widgets, legacy | Large SPA (no scoping) |
| **CSS Modules** | React/Vue SPA, component architecture | Static sites |
| **Tailwind** | Rapid prototyping, utility-first commands | Wix widgets, complex animations |
...
```css
/* ? Mobile-first: basic styles for mobile, then extend upward */
.grid {
  display: grid;
...
```css
/* Breakpoints (not custom properties — media queries not support var()) */
...
```css
/* ? Component adapts to the CONTAINER size, not the screen */
.card-container {
...
@container (min-width: 400px) {
  .card {
    flex-direction: row; /* Horizontal layout with enough space */
  }
}
```
...
### ✅ DO: dark mode via override tokens
...
### ? DON'T: invert colors or hardcode them
...
/* ❌ Duplication styles instead of overrides tokens */
.dark-mode .card {
  background: #1a1a2e;
  color: #eee;
  border-color: #333;
...
### ✅ DO: transition on concrete properties

```css
/* ✅ Explicit properties — performance, predictably */
.button {
  transition:
...
/* ? transition: all ? animates everything in a row, including width/height (layout thrashing) */
.button {
...
```css
/* ❌ Escalation z-index — impossible support */
.header   { z-index: 100; }
.dropdown { z-index: 999; }
.modal    { z-index: 9999; }
.toast    { z-index: 99999; }   /* someday 999999 will be needed */
```
...
```css
/* ✅ focus-visible — shows focus only with navigation keyboard */
...
/* ? Remove outline completely ? breaks keyboard navigation */
*:focus {
  outline: none;  /* ❌ Never */
}
```
...
```css
/* ✅ disable animations when user asks */
...
@media (forced-colors: active) {
  ...
  .icon {
    forced-color-adjust: auto;            /* Let the system recolor */
  }
}
```
...
```css
/* Minimal requirements WCAG 2.1 AA */
/* Regular text: contrast ≥ 4.5:1 */
...
| `transition: all` | List concrete properties |
| `outline: none` without replace | `focus-visible` + visible outline |
| Hardcode colors (`#333`, `red`) | CSS custom properties |
| `px` for font-size | `rem` (accessibility: respects user settings) |
| Global selectors (`div { }`) | Scoped: CSS Modules, BEM, or layer |
| Nested > 3 level (Sass) | Flat selectors, BEM |
...
## See also
- `$design_systems` — integration design systems (shadcn/ui, WDS)
- `$a11y_baseline` — full a11y checklist
- `$tailwind_beast_practices` — Tailwind-specific patterns
