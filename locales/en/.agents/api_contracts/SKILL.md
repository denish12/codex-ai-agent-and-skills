---
name: api_contracts
description: API contracts by UX flows — endpoints, schemas, errors, validation, authorization, perf/scalability considerations, idempotency and integration patterns.
---

# Skill: API Contracts

Full description API: from endpoint definition to error format.
...
7. [Example: full contract](#7-example)
...
| **Validation** | Zod schemas on each endpoint, on boundary (middleware) |
| **Auth** | Token in header or httpOnly cookie. Check on each protected endpoint |
| **Idempotency** | PUT = idempotent. POST create = idempotent key (if needed) |
| **Versioning** | URL prefix: `/api/v1/`, `/api/v2/` (with breaking changes) |
...
- **Never** not return stack trace, DB error, internal path
- `code` = machine-readable (for client)
- `message` = human-readable (for toast/UI)
- `details` = field-level errors (form validation)
...
|---------|-------|--------|
| **Cursor** | Large collections (> 1K), real-time data | Coupons list, activity log |
...
## 7. Example: full contract (Smart Cart Rescue)
