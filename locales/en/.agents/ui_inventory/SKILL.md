---
name: ui_inventory
description: Compose an inventory of UI components and reuse rules — basic components, composite components, patterns (forms, tables, dialogs), design (minimum).
---

# Skill: UI Components Inventory

Catalog UI components with variants, states, a11y notes.
...
5. Map to design system (section 5) if there is
6. Define design tokens (section 4)
7. Validate: each component in UI Spec coverage
```
...
Component needed ← UX Spec
    ├── There is in design? (WDS / shadcn / Mantine)
    │   ├── Yes, fits → USE (specify import + variant)
    │   ├── Yes, but needed customization → CUSTOMIZE (wrapper component)
    │   └── No → BUILD CUSTOM (with a11y baseline)
    └── Already is in project?
        ├── Yes → REUSE (specify path)
        └── No → CREATE (by template)
```
