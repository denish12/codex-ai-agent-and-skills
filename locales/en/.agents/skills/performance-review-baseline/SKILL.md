...
description: Basic perf review — N+1, pagination, batches, redundant queries, cache by architecture, heavy operations in the request path.
---

# Skill: Performance Review Baseline

Performance review without premature optimization — focus on real bottlenecks.
...
## See also
- `$observability-plan` — latency targets, metrics
- `$observability-review` — logging/metrics compliance
- `$data-model` — index strategy
- `$review-reference-snippets` — general DO/DON'T examples
