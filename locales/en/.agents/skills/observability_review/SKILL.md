...
description: Review observability — structured logs, request_id/trace_id, levels logs, ban leaks PII/secrets, compliance Observability Plan.
---

#Skill: Observability Review

Review observability implementation: logs, metrics, correlation, compliance with the plan.
...
## See also
- `$observability_plan` — what to log/measure/alert
- `$observability_logging` — implementation patterns (pino setup, middleware)
- `$review_reference_snippets` — DO/DON'T snippets (section N, O)
- `$code_review_checklist` — general review (observability section)