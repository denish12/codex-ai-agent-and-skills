...
Embed security into the design before coding.
...
|---------|---------|--------|
| **Defense in depth** | Multiple layers of defense | Validation + auth + sanitization |
| **Least privilege** | Minimal permissions | DB user: readWrite only, no admin |
| **Input validation at boundaries** | Trust no input | Zod schema on every endpoint |
| **Secure by default** | Secure "out of the box" | CORS whitelist, Helmet on, strict CSP |
| **Fail securely** | Error → secure state | Don't expose stack/DB errors |
| **Audit trail** | Log critical operations | Install, settings change, content create |
