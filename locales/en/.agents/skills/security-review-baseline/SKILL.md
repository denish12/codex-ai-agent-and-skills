...
Quick security review using a basic checklist. For a deep review → `$security-review`.

> **When to use:** Quick baseline during every review (5-10 min).
> **For deep review** of specific areas → `$security-review`.
...
For DO/DON'T examples → see `$review-reference-snippets`:
...
## Need deeper review?
→ Activate `$security-review` for: [auth / injection / SSRF / etc.]
```
...
- `$threat-model-baseline` — architecture-level threats