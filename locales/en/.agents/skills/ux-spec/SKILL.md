...
Specification that allows implementing UI without "guesswork".
...
7. [Example: Settings Screen](#7-example)
...
```
1. Input: UX Discovery Report (from $ux-discovery) + PRD
...
|------|------------|--------|
| Admin | Full CRUD, settings | Dashboard, admin panel |
| End User | View content, interact | Public-facing UI |
```
...
**Trigger:** User clicks "Add Item" in Items section
**Preconditions:** Logged in, authorized

| Step | Action | Result |
|------|--------|--------|
| 1 | Click "Add Item" button | Form appears |
| 2 | Fill name, value, type, expiry | Fields validated inline |
| 3 | Click "Save" | Loading state → toast "Saved" |

**Success:** Item appears in list
**Failure:** Validation errors shown inline
**Edge:** Duplicate name → error "Name already exists"
```
...
│   ├── Appearance tab (colors, fonts, layout)
│   └── Notifications tab (alerts, triggers)
├── Items
...
**Purpose:** Configure application appearance and behavior
**Role access:** Admin

### Layout
- [ ] Two-column: config panel (left) + preview (right)
...
| **Empty list** | Illustration + message + CTA ("Create your first item") |
...
| 3 | UI Inventory (`$ui-inventory`) | DEV | ☐ |
| 4 | A11y Baseline (`$a11y-baseline`) | DEV + Tester | ☐ |
...
## 7. Example: Settings Screen

```markdown
## Screen: Settings

**URL/Route:** /admin/settings
**Purpose:** Configure application appearance
**Role:** Admin

### Layout
Two-column: Config Panel (left, 60%) + Preview (right, 40%)
...
| Appearance | Appearance | ColorPickers (4 tokens), FontSelect |
| Notifications | Notifications | Switch (enable), frequency Select |

### States
| State | Loading | Empty | Error | Success |
|-------|---------|-------|-------|---------|
| Config | Skeleton fields | N/A (defaults) | Inline + toast | Toast "Saved" |
| Preview | Skeleton preview | Default template | Error illustration | Live update |
```
...
## See also
- `$ux-discovery` — discovery before spec
- `$ui-inventory` — component inventory
- `$a11y-baseline` — accessibility requirements
- `$design-intake` — analyze design materials
- `$google-stitch-skill` — generate UI screens from spec
