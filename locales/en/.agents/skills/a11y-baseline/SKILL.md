---
name: a11y-baseline
description: Minimum accessibility baseline for web UI — keyboard navigation, focus management, form labels, ARIA for interactive components, error messages.
---

# Skill: A11y Baseline

Minimum accessibility requirements for an MVP. For testing → `$qa-ui-a11y-smoke`.
...
## See also
- `$qa-ui-a11y-smoke` — a11y smoke testing
- `$ui-inventory` — component inventory (includes a11y notes)
- `$styling-css-stack` — CSS a11y patterns (focus-visible, prefers-reduced-motion)
- `$ux-spec` — UX Spec (screens with states)