...
Create ADR for **each non-trivial choice** — if in 6 months someone asks "why like this?":
...
## Context
<Why is this needed? What constraints? What hurts?>
- Current situation
- Requirements / constraints
- What happens if it is not solved

##Decision
<What we chose and how exactly we will do it>
...
| Title: action + object | "Use MongoDB as primary database" |
| File: `ADR-XXX-kebab-case.md` | `ADR-001-use-mongodb.md` |
| Directory: `docs/architecture/adr/` | — |
| Chronological order | New ADR = next number |
...
For complex decisions with 3+ alternative use weighted matrix:
...
| Status | Meaning |
|--------|---------|
| **Proposed** | Decision is proposed, waiting for review |
| **Accepted** | Decision is accepted and applied |
| **Deprecated** | Decision is deprecated, but the code still uses it |
| **Superseded** | Superseded by another ADR (specify link) |
...
|----------------|-----------|
| ADR without Alternatives | Always at least 2 alternatives |
| "We chose X because it's popular" | Concrete pros/cons by project criteria |
| 500-line ADR | Short and concrete: max 50-80 lines |
| ADR retroactively (a month later) | Create ADR in the moment the decision is made |
| No cross-references | Links to the PRD, related ADRs, tickets |
| No Negative consequences | Every decision has trade-offs |
| "Alternatives: none" | If there are no alternatives — revisit the scope question |
...
- `$system_design_checklist` — checklist design completeness
