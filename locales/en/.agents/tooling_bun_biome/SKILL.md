...
description: Tooling — Biome (lint/format) and Bun (runtime/package manager). biome.json config, key rules, Bun scripts, pre-commit hooks, CI pipeline, migration with ESLint+Prettier. Activate with setup tooling, add a linter, or with questions «how configure Biome/Bun».
---

# Skill: Tooling (Bun + Biome) Beast Practices

Concrete configs, commands and DO/DON'T for Biome (lint + format) and Bun (runtime + package manager).

**Sections:**
1. [Biome: setup and config](#1-biome-setup)
2. [Biome: key rules](#2-key)
...
7. [Migration with ESLint + Prettier](#7-migration)
...
// apps/dashboard/biome.json (extends root)
{
  ...
  "linter": {
    "rules": {
      "suspicious": {
        "noConsoleLog": "off"  // allow console.log in the dashboard
      }
    }
  }
}
...
| Rule | Level | Why important |
|---------|---------|-------------|
| `useConst` | error | Non-mutated variables = `const` |
| `noVar` | error | `var` — hoisting bugs |
| `noDoubleEquals` | error | `==` — implicit type coercion |
...
### ✅ DO: configure ignore for specific code

```js
// ✅ Targeted ignore with justification
// biome-ignore lint/suspicious/noConsoleLog: Intentional debug output for dev
console.log('Startup config:', config);

// ❌ Do not ignore broadly without reasons
// biome-ignore lint: TODO fix later  ← ❌ no reasons
```
...
| Built-in test runner | `bun test` (Jest-compatible) | Needed vitest/jest |
| Built-in bundler | `bun build` | Needed webpack/vite |
| Compatibility | 99%+ npm compatible | 100% |
| CI/CD | Bun setup required | Preinstalled |
| Production | Growing, but npm is more stable | Industry standard |

> [!IMPORTANT]
> Use Bun only if the project decision requires it. For production → npm.
> Bun excellent for dev-loop: install, test, scripts.

### ✅ DO: main Bun commands
...
| `biome lint .` | Only lint (check) | Review |
| `biome lint --write .` | Lint with autofix | Dev |
| `biome format --write .` | Only format | Dev |
...
### ✅ DO: lefthook (faster husky + lint-staged)
...
## 7. Migration with ESLint + Prettier
...
# 8. Update pre-commit hooks (if there is)
...
|----------------|-----------|
| ESLint + Prettier + Biome simultaneously | Choose one: Biome replace both |
| `biome-ignore` without reasons | Always specify the reason after `:` |
| Different formatter settings in IDE and CI | Unified `biome.json`, IDE reader |
| `npm run lint:fix` in CI | CI: `biome ci .` (without fix, strict mode) |
| Ignore warnings | Warning → error in CI via `biome ci` |
| Several lockfile (package-lock.json + bun.lockb) | One package manager, one lockfile |
| `bun` in productionuction without tests compatibility | npm for prod, Bun for dev-loop |
...
## See also
- `$es2025_beast_practices` — modern JavaScript (rules align with Biome rules)
- `$testing_strategy_js` — Vitest configuration
