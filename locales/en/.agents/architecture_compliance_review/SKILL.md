...
description: Check compliance code architecture/ADR — boundaries modules, layers, dependencies, conventions, red flags.
---

#Skill: Architecture Compliance Review

Review on compliance with the architecture document and ADR.
...
|---|----------|-------|-----------|---------|-----|
| 1 | 🔴 P0 | ARCH-01 | `routes/coupons.js:25` | `Coupon.find()` called directly in route | Move to `couponService.list()` |
| 2 | 🟠 P1 | ARCH-10 | `services/settings.js` | 620 lines — God Object risk | Split into `settingsService` + `templateService` |
...
## See also
- `$architecture_doc` — Architecture Document (reference)
- `$adr_log` — ADR Log (reference)
- `$code_review_checklist` — general review (architecture section)
- `$current_state_analysis` — initial repo audit
