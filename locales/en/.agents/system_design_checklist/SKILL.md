...
description: Checklist system design — functional/non-functional requirements, technical design, operations (deploy/monitoring/backup/rollback), so that nothing not forget.
---
...
1.3 API contracts: ✅ PASS
  POST /api/v1/coupons — CreateCouponRequest → CouponResponse (201)
  GET /api/v1/coupons/:id — → CouponResponse (200) | ErrorResponse (404)
...
1.5 Edge cases: ❌ MISSING
  Behavior is not defined for:
  - Concurrent update settings (optimistic locking?)
  - Expired coupon on storefront (show expired or hide?)
  → Recommendation: add acceptance criteria in PRD
  → Task: ARCH-05
```
...
| 2.3 | Availability | Uptime target (99.9%?). Allowed downtime. Graceful degradation strategy | ☐ |
...
Measurement: response time from gateway, without network latency.
```
...
| # | Check | Guidance | Status |
|---|-------|---------|--------|
| 3.1 | Architecture diagram | Text (mermaid/ASCII). Shows: components, flows data, boundaries | ☐ |
| 3.2 | Component responsibilities | For each: what does, what Not does, public API, dependencies | ☐ |
...
| 3.5 | Error handling | Unified format errors. Mapping: domain errors → HTTP codes → UI messages | ☐ |
...
| 4.2 | Embed vs Reference | Intentional choice, documented in ADR | ☐ |
| 4.3 | Indexes | Compound indexes under key query patterns. Verified via explain() | ☐ |
| 4.4 | Pagination | Cursor for large collections, skip for small | ☐ |
| 4.5 | Migrations | migrate-mongo or analogue. Backfill strategy for schema changes | ☐ |
...
| # | Check | Guidance | Status |
|---|-------|---------|--------|
| 5.1 | Input validation | Zod schemas on each endpoint. Whitelist approach | ☐ |
...
| 6.6 | Rollback plan | How to roll back: previous Docker image, DB migration down, feature flag | ☐ |

---

## 7. Red Flags

| # | Flag | What this | How to detect |
|---|------|---------|---------------|
...
| 7.3 | Tight Coupling | Change one module breaks other | No interfaces/contracts between layers |
| 7.4 | Magic | Non-obvious behavior without documentation | Mongoose hooks with business logic, implicit globals |
| 7.5 | Analysis Paralysis | Endless planning | No MVP paths, 3+ alternatives without decisions |
| 7.6 | Premature Optimization | Optimization without data | Cache/sharding before the first user |
| 7.7 | Not Invented Here | Refusal of ready-made solutions | Custom auth, custom ORM, custom logger |
...
## See also
- `$current_state_analysis` — audit current states (before this checklist)
- `$architecture_doc` — full Architecture Document (after checklist)
- `$adr_log` — recording decisions
- `$security_baseline_dev` — detail security check
