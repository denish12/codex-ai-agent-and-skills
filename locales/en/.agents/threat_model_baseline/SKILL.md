...
description: Baseline threat model (threat baseline) for MVP — assets, attack vectors, mitigations. Includes OWASP-risks and roles/permissions.
---
...
8. [Example: Smart Cart Rescue](#8-example)
...
|---------|---------|--------|
| **Defense in depth** | Several layers protection | Validation + auth + sanitization |
| **Least privilege** | Minimal permissions | DB user: readWrite only, no admin |
| **Input validation at boundaries** | Not trust any input | Zod schema on each endpoint |
| **Secure by default** | Safely "out of the box" | CORS whitelist, Helmet on, strict CSP |
| **Fail securely** | Error → secure state | Don't expose stack/DB errors |
| **Audit trail** | Log critical operations | Install, settings change, coupon create |
...
| **Coupon codes** | Medium | Unauthorized discount usage |
...
| **Dashboard API** | Settings/Coupons CRUD | High |
...
For each threat: description, impact, likelihood, severity, mitigation.
...
| T-06 | **Brute force coupon codes** | Widget endpoint | Medium | Low | 🟡 P2 | Rate limiting on widget endpoint | ☐ |
...
## 8. Example: Smart Cart Rescue

```markdown
# Threat Model: Smart Cart Rescue

## Key Assets
- Wix OAuth tokens, appInstanceId
- Coupon codes, settings data
- DB credentials

## Top Threats (prioritized)
1. 🔴 T-01: NoSQL injection via dashboard API → Zod + safe filter
2. 🔴 T-04: IDOR on settings/coupons → verify appInstanceId ownership
3. 🔴 T-05: Secrets in code → env vars + pino redact
4. 🟠 T-03: Webhook spoofing → JWT verification
5. 🟠 T-10: Embed script injection → trusted origin, no eval
...
## See also
- `$security_baseline_dev` — implementation patterns (Zod, Helmet, bcrypt)
- `$security_review` — AppSec review checklist
- `$api_contracts` — auth/error conventions
- `$observability_logging` — PII redaction, audit logging
