---
name: data_model
description: Model data and strategy store — normalization/denormalization, indexes, transactions, cache, (optional) event sourcing/eventual consistency — with trade-offs and tied to UX flows.
---

# Skill: Data Model

Model data: from entity definitions to query optimization.
...
9. [Example: full model](#9-example)
...
### Decision table (for project)

| Parent | Child | Pattern | Rationale |
|--------|-------|---------|-----------|
| Installation | Settings | **Embed** (or 1:1 ref) | Always are read together, 1 settings per install |
...
> Each choice embed/reference = a potential ADR. If it is not obvious — create `$adr_log`.
...
| Rule | Description |
|---------|---------|
| **ESR** | Equality → Sort → Range (order fields in compound) |
| **Max 5-7** | Each index = write overhead |
| **Verify** | `explain('executionStats')` → IXSCAN, not COLLSCAN |
| **Partial** | If index needed not for all docs: `partialFilterExpression` |
...
> MongoDB any modeling, where transactions needed **rarely**. If transaction needed often — revisit schema (can is worth embed).
...
| When | How |
|-------|-----|
| New field (optional) | Add in schema + `$setOnInsert` default |
