...
description: Quick UI/a11y smoke review design — states loading/empty/error, focus, keyboard, aria/labels, compliance UX Spec. Execution To transfer in QA.
---

# Skill: UI & A11y Smoke Review (Designer-Level)

Quick designer-level UI check for compliance with the UX Spec + a11y baseline **before handoff to QA**.

> **Difference from `$qa_ui_a11y_smoke`:** This skill — design quick check (5-10 min, by UX Spec).
> `$qa_ui_a11y_smoke` — form QA test (25 WCAG checks, tools, report).
...
1. Input:
   ...
   ├── UX Spec (from $ux_spec) — source of truth
   └── A11y Baseline (from $a11y_baseline) — requirements
...
## Next
- If READY → pass to $qa_ui_a11y_smoke formal testing
