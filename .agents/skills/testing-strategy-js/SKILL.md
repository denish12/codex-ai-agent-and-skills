...
> Подробнее по E2E → `$qa-e2e-playwright`.
...
/**
 * Создаёт тестовый объект продукта с возможностью override.
 * @param {Partial<Product>} overrides - поля для переопределения.
 * @returns {Product} тестовый продукт.
 */
...
├── components/
│   ├── SearchInput.jsx
│   └── SearchInput.test.jsx     # RTL — рядом с компонентом
├── api/
│   ├── products.handler.js
│   └── products.integration.test.js  # Integration — рядом с маршрутом
...
## См. также
- `$tests-quality-review` — ревью качества тестов
- `$qa-e2e-playwright` — E2E через Playwright
- `$qa-browser-testing` — визуальное тестирование в Antigravity
- `$es2025-beast-practices` — современный JS для написания чистого кода
- `$dev-reference-snippets` — TDD, React, API примеры