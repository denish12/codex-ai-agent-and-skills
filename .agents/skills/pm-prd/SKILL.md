...
PRD, по которому UX/ARCH/DEV/TEST могут работать без «догадок».
...
```
1. Input: Interview Summary (из $pm-interview)
...
| **Accessibility** | WCAG 2.1 AA baseline (see $a11y-baseline) | P1 |
...
| 5 | Backlog (`$pm-backlog`) | DEV + Conductor | ☐ |
...
## См. также
- `$pm-interview` — interview/discovery (input)
- `$pm-backlog` — decompose PRD into backlog
- `$ux-discovery` → `$ux-spec` — UX design (uses PRD)