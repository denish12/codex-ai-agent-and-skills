...
> Любой output требует доработки в Figma и ревью через `$design-parity-review`.
...
6. Сверка с $design-parity-review
...
- [ ] Пройдена сверка через `$design-parity-review` → ≤0 P0
...
| Screen Inventory | `generate_screen_from_text` для каждого экрана → `get_screen` | `$design-intake` |
| Handoff → DEV | `get_screen` → код как scaffold + Figma-финал | — |
| Parity Review | Сравнивать реализацию с **Figma**, не со Stitch-output | `$design-parity-review` |
...
- UX-xx: сверка через `$design-parity-review`
...
- **Связанные skills:** `$design-intake`, `$design-parity-review`
