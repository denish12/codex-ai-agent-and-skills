...
|---|----------------|---------|-------------|
| 1 | A3 (error paths) | Нет тестов на сетевые ошибки в fetchItems | Добавить тест с reject mock |
...
| # | Пункт чеклиста | Проблема | Рекомендация |
|---|----------------|---------|-------------|
| 1 | E2 (snapshot size) | Snapshot на 200 строк в ProductForm.test | Перейти на inline snapshot или RTL assertion |
...
## См. также
- `$testing-strategy-js` — стратегия и паттерны написания тестов
- `$code-review-checklist` — общий чеклист code review
- `$qa-e2e-playwright` — E2E через Playwright
- `$dev-reference-snippets` — примеры кода
