---
name: qa-manual-run
description: Ручной прогон критичных сценариев и регресса — happy/edge/error paths по UX Spec; фиксация багов с шагами и доказательствами.
---

# Skill: QA Manual Run

Ручной прогон тест-кейсов с документированием результатов.
...
```
1. Подготовка
   ├── Прочитай Test Plan (из $qa-test-plan)
   ├── Проверь entry criteria (CI green, env up, data seeded)
   └── Подготовь test data / fixtures

2. Прогон
   ├── Пройди каждый test case по приоритету (P0 → P1 → P2)
   ├── Для каждого: выполни steps, проверь expected result
   ├── Зафиксируй: PASS / FAIL / BLOCKED / SKIPPED
   └── Для FAIL → создай bug report (section 3)

3. Регресс
   └── Перепройди ранее закрытые баги (проверь fix не сломал)

4. Отчёт
   └── Заполни output template (section 5)
```
...
Use `$qa-browser-testing` workflow:
...
## См. также
- `$qa-test-plan` — create test plan before running
- `$qa-browser-testing` — browser-based visual testing with Antigravity
- `$qa-api-contract-tests` — API-level testing
- `$qa-security-smoke-tests` — security smoke tests
