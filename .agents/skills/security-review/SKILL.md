...
Глубокое ревью безопасности кода: от secrets до injection.
...
| Триггер | Depth |
|---------|-------|
| Новый API endpoint | Full review |
...
## См. также
- `$security-baseline-dev` — implementation patterns (Zod, Helmet, safe filter)
- `$threat-model-baseline` — architecture-level threat model
- `$code-review-checklist` — general code review
- `$review-reference-snippets` — more DO/DON'T examples