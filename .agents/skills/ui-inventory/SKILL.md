...
Каталог UI компонентов с variants, states, a11y notes.
...
```
1. Input: UX Spec (из $ux-spec)
2. Extract все упомянутые компоненты из Screen Specs
3. Classify: atom / molecule / organism / pattern
4. For each → Component Card (section 2)
5. Map to design system (section 5) если есть
6. Define design tokens (section 4)
7. Validate: каждый компонент в UI Spec покрыт
```

### Decision: Use existing vs Build custom

```
Компонент нужен ← UX Spec
    ├── Есть в дизайн-системе? (WDS / shadcn / Mantine)
    │   ├── Да, подходит → USE (указать import + variant)
    │   ├── Да, но нужна кастомизация → CUSTOMIZE (wrapper component)
    │   └── Нет → BUILD CUSTOM (с a11y baseline)
    └── Уже есть в проекте?
        ├── Да → REUSE (указать путь)
        └── Нет → CREATE (по template)
```
...
## См. также
- `$ux-spec` — Screen specs (input for inventory)
- `$a11y-baseline` — a11y requirements per component
- `$design-systems` — integration guide for design systems
