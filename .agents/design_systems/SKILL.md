...
## См. также
- `$ui_inventory` — полный формат UI inventory
- `$styling_css_stack` — CSS токены и naming
- `$ux_spec` — UX спецификация (определяет состояния)
- `$a11y_baseline` — accessibility чеклист
