...
> [!WARNING]
> Для Smart Cart Rescue — мы **используем** legacy iFrame SDK, потому что Dashboard component работает как iframe в Wix. Миграция на Dashboard SDK возможна, но требует переписать всю коммуникацию Settings Panel ↔ Widget.
...
## См. также
- `$wix_self_hosted_embedded_script` — Embedded Script extension (использует iFrame SDK для Settings Panel)
- `$react_15_3_wix_iframe` — React 15.3 patterns для Wix iFrame (если применимо)
