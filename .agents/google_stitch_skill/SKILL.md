...
> [!TIP]
> Для popup-шаблонов SCR используй `AGNOSTIC` — попапы не привязаны к конкретному устройству.
...
### SCR-проекты в Stitch

| Проект | Project ID | Экранов | Описание |
|--------|-----------|---------|----------|
| Wix Native Style Popup | `9413055351550649236` | 90+ | Все 12 popup-шаблонов + dashboard + onboarding |
| Rules & Triggers Wix Redesign | `4150501200823065821` | 7 | Rules & Triggers UI |

### Именованные экраны проекта Popups
...
> [!TIP]
> Используй эти ID для `edit_screens` и `generate_variants` — не нужно создавать экраны заново.
> Пример: `edit_screens(projectId: "9413055351550649236", selectedScreenIds: ["ada37869..."], prompt: "...")`
...
### Сценарий 4: SCR Popup Template
```
Цель: Сгенерировать/обновить popup-шаблон для Smart Cart Rescue
Модель: GEMINI_3_FLASH → GEMINI_3_PRO
Device: AGNOSTIC
Шаги:
1. Взять Template Brief из agents_documentation/popup_description/
2. Извлечь: уникальные элементы, цвета, стиль из секций 3–6
3. generate_screen_from_text(prompt с popup + prop-элементы, AGNOSTIC)
...
DoD: Popup визуально соответствует HTML-референсу, все prop-элементы видны
```

---

## Примеры промптов для Smart Cart Rescue

> Полная коллекция промптов: `examples/scr_prompts.md`
