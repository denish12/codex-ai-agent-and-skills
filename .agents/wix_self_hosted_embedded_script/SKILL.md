...
| Компонент | Роль | URL |
|-----------|------|-----|
| **Dashboard** | Управление настройками и купонами | `/dashboard` |
...
export const getWidgetPayload = asyncHandler(async (req, res) => {
  ...
  // 2. Загрузить latest active coupon
...
| Script загрузился, UI нет | JS runtime error или пустой payload | Browser console → `[SCR]` messages |
...
| Coupon не показывается | Нет active coupon в DB | Проверить `coupons` коллекцию, `active: true` |
...
- [ ] Embedded script реально внедрён в сайт (виден в DOM: #scr-widget-root)
...
- [ ] CTA выполняет action (copy coupon code / redirect)
- [ ] Coupon показывается из DB (latest active)
...
## См. также
- `$node_express_beast_practices` — Express архитектура
- `$security_baseline_dev` — безопасная обработка webhooks
- `$observability_logging` — structured logs для debugging
