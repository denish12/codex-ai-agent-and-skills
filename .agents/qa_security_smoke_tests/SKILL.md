...
Прогон security smoke: auth, validation, error exposure, rate limiting.
...
```
1. Определи protected endpoints (из $api_contracts)
2. Для каждого protected endpoint:
   a. Запрос без auth → expect 401
   b. Запрос с чужим auth → expect 403/404
   c. Невалидный input → expect 400 (не 500!)
   d. Проверь error response: нет stack/DB/PII
3. Для public endpoints:
   a. Проверь rate limiting (если включен)
4. Проверь логи: нет PII/секретов
5. Заполни results table
```
